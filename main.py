...
def _menu_principal(estado: config.EstadoApp) -> None:
    ...
    while True:
        ...
        opcoes: list[Any] = [
            ...
            ("12) 🧽  Debloat do Windows (Copilot, bloatware, telemetria)", "debloat"),
            interface.separador("Por jogo"),
            ("13) 🎯  Otimizar para um jogo (detecta no PC)", "otimizar_jogo"),
            interface.separador("⭐ Premium"),
            ("14) 🏆  Pontuação de saúde do PC", "saude"),
            ("15) ⚡  Perfis de otimização (1 clique)", "perfis"),
            ("16) 📊  Antes e depois (desempenho)", "desempenho"),
            ("17) 📄  Gerar relatório (HTML)", "relatorio"),
            ("18) 🗓️  Manutenção automática (agendar limpeza)", "agendador"),
            interface.separador("Ferramentas"),
            (f"19) 🧪  Modo simulação ({'desligar' if estado.simulacao else 'ligar'})", "simulacao"),
            ("20) ↩  Desfazer última alteração", "desfazer"),
            ("21) ⏮  Reverter TUDO (todas as alterações)", "reverter_tudo"),
            ("22) ⚙  Preferências", "preferencias"),
            ("23) 📜  Ver logs", "logs"),
            ("24) 🔄  Verificar atualizações", "atualizacao"),
            ("0)  🚪  Sair", "sair"),
        ]
        ...
        acoes: dict[str, tuple[str, Callable[[], None]]] = {
            ...
            "debloat": ("Debloat do Windows", lambda: debloat.menu(estado)),
